        ...
        stage('Deploy to Kubernetes (EKS)') {
            ...
            steps {
                script {
                    ...
                    sh "sed 's/\\${IMAGE_TAG}/${IMAGE_TAG}/g' k8s/frontend/deployment.yaml | kubectl apply -f -"
                    
                    echo "Deploying Auth API..."
                    sh "sed 's/\\${IMAGE_TAG}/${IMAGE_TAG}/g' k8s/auth-api/deployment.yaml | kubectl apply -f -"
                    
                    echo "Deploying ML API..."
                    sh "sed 's/\\${IMAGE_TAG}/${IMAGE_TAG}/g' k8s/ml-api/deployment.yaml | kubectl apply -f -"
